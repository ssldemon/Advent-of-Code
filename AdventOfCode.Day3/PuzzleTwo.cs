...
public class PuzzleTwo
{
    ...
    int OxygenGeneratorRating(string[] diagnosticReport)
    {
        ...
        for (var i = 0; diagnosticReport.Length > 1 && i < lineLength; i++)
        {
            var bit = 2 * diagnosticReport.Count(line => line[i] == '1') >= diagnosticReport.Length ? '1' : '0';
            diagnosticReport = diagnosticReport.Where(line => line[i] == bit).ToArray();
        }
    ...
    int Co2ScruberRating(string[] diagnosticReport)
    {
        ...
        for (var i = 0; diagnosticReport.Length > 1 && i < lineLength; i++)
        {
            var bit = 2 * diagnosticReport.Count(line => line[i] == '1') >= diagnosticReport.Length ? '0' : '1';
            diagnosticReport = diagnosticReport.Where(line => line[i] == bit).ToArray();
        }
