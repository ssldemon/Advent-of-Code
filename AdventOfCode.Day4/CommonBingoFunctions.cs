...
namespace AdventOfCode.Day4
{
    ...
    {
        public List<string[,]> PopulateBingCards()
        {
        ...
        public List<string> BingoBallPulls()
        {
            string[] file = System.IO.File.ReadAllLines(@"Bingo.txt");
        ...
        public string[,] PullBingoEntry(string[,] card, string numberToRemove)
        {
            List<int> index = new();
            int count = 0;
        ...
        public bool CheckForWinner(string[,] card)
        {
            for (int i = 0; i <= 4; i++)
            {
                List<string> row = new();
        ...
        public void BingoPrint(string[,] card)
        {
            Console.WriteLine();
            for (int col = 0; col < card.GetLength(0); col++)
            {
